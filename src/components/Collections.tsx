...
const Collections: React.FC = () => {
  ...
  const getColorClasses = (color: string) => {
    const colorMap = {
      'journal-lavender': 'bg-journal-lavender-light border-journal-lavender text-journal-lavender',
      'journal-peach': 'bg-journal-peach-light border-journal-peach text-journal-peach',
      'journal-sage': 'bg-journal-sage-light border-journal-sage text-journal-sage',
      'journal-stone': 'bg-journal-stone-light border-journal-stone text-journal-stone',
    };
        ...
        {sampleCollections.map((collection) => (
          ...
          >
            ...
            <div className="flex items-start justify-between mb-4">
              <div className={`p-3 rounded-xl ${getColorClasses(collection.color).split(' ')[0]}/20`}>
                <BookOpen size={24} className={getColorClasses(collection.color).split(' ')[2]} />
              </div>
              <div className={`px-3 py-1 rounded-full text-xs font-medium ${getColorClasses(collection.color)}/20 ${getColorClasses(collection.color).split(' ')[2]}`}>
            ...
            <div className="space-y-2">
              ...
              {collection.recentItems.map((item, index) => (
                ...
                >
                  <div className={`w-1.5 h-1.5 rounded-full ${getColorClasses(collection.color).split(' ')[2].replace('text-', 'bg-')}`}></div>
                  {item}
                </div>
              ))}
            </div>
