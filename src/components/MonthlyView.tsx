...
const MonthlyView: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-white/20">
        ...
        <div className="grid grid-cols-7 gap-2 mb-4">
          {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
            <div key={day} className="text-center font-semibold text-journal-stone/70 py-2">
              {day}
            </div>
          ))}
        </div>
          ...
          {days.map((day, index) => (
            <div
              ...
              className={`
                ...
                ${isToday(day) ? 'bg-journal-sage text-white shadow-md' : 'bg-journal-cream-dark/30'}
              `}
            >
              {day && (
                <div className="h-full flex flex-col">
                  <div className={`font-semibold mb-1 ${isToday(day) ? 'text-white' : 'text-journal-stone'}`}>
                    {day}
                  </div>
                  ...
                  {sampleEvents[day as keyof typeof sampleEvents] && (
                    <div className="space-y-1 flex-1">
                      {sampleEvents[day as keyof typeof sampleEvents].map((event, i) => (
                        <div
                          key={i}
                          className={`
                            text-xs px-2 py-1 rounded-md truncate
                            ${isToday(day) ? 'bg-white/20 text-white' : 'bg-journal-lavender/20 text-journal-stone'}
                          `}
                          title={event}
                        >
                          {event}
      ...
      {/* Monthly Goals */}
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-white/20">
        <h3 className="text-lg font-semibold text-journal-stone mb-4">Monthly Goals</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            <div key={index} className="flex items-center gap-3 p-3 rounded-xl bg-journal-cream-dark/50">
